...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'developer.update': {
    roles: ['game-owner', 'game-tester', 'customer-support'],
    key: 'developer-profile-update',
    tenantPath: ['developer'],
    userPath: [],
  },
  'game.create': {
    roles: ['game-owner', 'game-tester'],
    key: 'new-game-level',
    tenantPath: ['developer', 'game'],
    userPath: [],
  },
  'game.update': {
    roles: ['game-owner', 'game-tester'],
    key: 'game-level-update',
    tenantPath: ['developer', 'game'],
    userPath: [],
  },
  'game.delete': {
    roles: ['game-owner', 'game-tester'],
    key: 'game-level-delete',
    tenantPath: ['developer', 'game'],
    userPath: [],
  },
  'issue.create': {
    ...
    key: 'developer-issue',
    tenantPath: ['developer', 'user', 'customer_support', 'issue'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['level-designer'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['game-owner', 'level-designer', 'game-tester', 'customer-support'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.developer.findFirst({
