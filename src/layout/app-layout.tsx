...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['level-designer', 'game-owner', 'game-tester', 'customer-support'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<DeveloperInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/developers` : null),
    () => getDevelopers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Developers',
      path: isTenantUser ? `/developers/view/${data?.[0]?.id}` : '/developers',
      entity: 'developer',
