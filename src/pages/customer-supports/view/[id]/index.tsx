...
import { getCustomerSupportById } from 'apiSdk/customer-supports';
import { CustomerSupportInterface } from 'interfaces/customer-support';
import { IssueListPage } from 'pages/issues';

function CustomerSupportViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<CustomerSupportInterface>(
    () => (id ? `/customer-supports/${id}` : null),
    () =>
      getCustomerSupportById(id, {
        relations: ['user'],
      }),
  );
        ...
        {isLoading ? (
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('customer_support', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/customer-supports/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Issue Id" text={data?.issue_id} />
            ...
            <Box borderRadius="10px" border="1px" borderColor={'base.300'} mt={6}>
              <IssueListPage
                filters={{ customer_support_id: id }}
                hidePagination={true}
