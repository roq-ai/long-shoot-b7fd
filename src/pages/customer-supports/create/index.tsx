...
import { createCustomerSupport } from 'apiSdk/customer-supports';
...
function CustomerSupportCreatePage() {
  const router = useRouter();
  const [error, setError] = useState(null);

  const handleSubmit = async (values: CustomerSupportInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
  ...
  const formik = useFormik<CustomerSupportInterface>({
    initialValues: {
      issue_id: '',
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <Flex justifyContent={'flex-start'}>
            ...
            <Button
              ...
              gap="0.5rem"
              mr="4"
              onClick={() => router.push('/customer-supports')}
              _hover={{
                bg: 'neutral.transparent',
                color: 'neutral.main',
              }}
