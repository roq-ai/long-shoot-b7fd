...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getGameOwners();
    case 'POST':
      return createGameOwner();
    default:
  ...
  async function getGameOwners() {
    ...
    const response = await prisma.game_owner
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'game_owner'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createGameOwner() {
    ...
    const data = await prisma.game_owner.create({
      data: body,
    });
