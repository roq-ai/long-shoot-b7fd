...
import { getCustomerSupports } from 'apiSdk/customer-supports';
...
import { getGameOwners } from 'apiSdk/game-owners';
import { GameOwnerInterface } from 'interfaces/game-owner';

import { IssueListPage as ShowCaseList } from 'pages/issues';

function DashboardPage() {
  const { session } = useSession();

  const customerSupportStat = useSWR<PaginatedInterface<CustomerSupportInterface>>(
    () => `/customer-supports`,
    () => getCustomerSupports({ limit: 1, offset: 0 }),
  );
  ...
  const gameOwnerStat = useSWR<PaginatedInterface<GameOwnerInterface>>(
    () => `/game-owners`,
    () => getGameOwners({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Customer Supports',
      result: customerSupportStat?.data?.totalCount,
      isLoading: customerSupportStat.isLoading,
    ...
    {
      title: 'Game Owners',
      result: gameOwnerStat?.data?.totalCount,
      isLoading: gameOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
